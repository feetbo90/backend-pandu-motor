...
module.exports = {
  ...
  async create(req, res) {
    try {
      ...
      res.status(201).json({
        message: "Data sirkulasi_stock berhasil ditambahkan",
        data
      });
    } catch (err) {
      res.status(500).json({
  ...
  // GET /api/sirkulasi-stock?branch_id=1&year=2025&month=6
  async getAll(req, res) {
    try {
      ...
      res.status(200).json({
        message: "Data sirkulasi_stock berhasil diambil",
  ...
  async update(req, res) {
    try {
      ...
      const data = await SirkulasiStock.findByPk(id);
      if (!data) return res.status(404).json({ message: "Data tidak ditemukan" });
      ...
      res.status(200).json({
        message: "Data sirkulasi stock berhasil diperbarui",
        data
      });
    } catch (err) {
      res.status(500).json({
