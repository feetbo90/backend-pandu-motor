...
module.exports = {
  ...
  async create(req, res) {
    try {
      const { branch_id, total, total_piutang, month, year, ...rest } = req.body;

      if (!branch_id) {
        return res.status(400).json({ message: "branch_id wajib diisi" });
      }

      const existing = await SirkulasiPiutang.findOne({
      ...
      if (existing) {
        if (existing.is_active === true) {
          ...
          return res.status(400).json({
            message: `Data sirkulasi piutang untuk tahun ${year} dan bulan ${month} sudah aktif`
          });
        } else {
          ...
          return res.status(200).json({
            message: "Data sirkulasi piutang nonaktif berhasil diaktifkan kembali dan diperbarui",
            data: existing
          });
        }
      }
